...
draw_brace <- function(dat_trend, upper_label, lower_label, year_vec, bundesland){
  ...
  brace_coordinates <- dat_trend %>%
    ...
    mutate(label_pos = ifelse(KBuecher_imp3 == 1, range_est[1] - 113, range_est[1] - 133)) %>%
    mutate(sigTrend_vsGermany = ifelse(pTrend_vsGermany < 0.05, "<sup>a</sup>", "")) %>%
    mutate(estTrend_within_label = ifelse(sigTrend_within == "bold", paste0("**", round(estTrend_within, 0), "**"), round(estTrend_within, 0)))
