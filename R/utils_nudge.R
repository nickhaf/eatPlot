...
calc_brace_coords <- function(grouping_var_lvls, coords, years_list, plot_settings = plotsettings_lineplot()) {
  ...
  # calc brace label y ------------------------------------------------------
  brace_positions$label_pos_x <- brace_positions$year_start + brace_positions$range * brace_positions$brace_position_x + (max(brace_positions$range, na.rm = TRUE) * plot_settings$brace_label_nudge_x)
