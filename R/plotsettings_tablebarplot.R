...
#' @param bar_label_nudge_out Numeric. Necessary for stacked barplots. If the percentage of a subgroup lies below that value, the label will be nudged to the left or right of the stacked bar. Defaults to 5.
...
#' @param headers_ggtext Logical indicating whether the headers should be drawn with `ggtext::geom_richtext()`. Defaults to `TRUE`. Only set to `FALSE` if you want to plot greek letters in the headers AND save the plot in cmyk color format afterwards. When set to FALSE,`grDevices::plotmath()` will be used for plotting the special characters, so refer to the documentation of the `grDevices::plotmath()`-function for the syntax.
