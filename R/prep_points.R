...
## Bundesland vs. the whole Group (Germany).
  est_rows <- c(## Germany:
              grep("^0$", data$group), grep("^1$", data$group), grep("^wholeGroup", data$group),
  ...
  data_est[is.na(data_est$TR_BUNDESLAND), "TR_BUNDESLAND"] <- rep("wholeGroup", length(which(is.na(data_est$TR_BUNDESLAND))))
