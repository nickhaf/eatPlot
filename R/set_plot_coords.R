...
calc_y_value_coords <- function(y_range,
                                ...
                                nudge_param_lower = 0.4) { # nudge_param increases the distance between lowest/highest point and braces/x axis
  y_range_diff <- diff(y_range)

  coords <- c(
    ## Lower y limit
    y_range[1] - y_range_diff * nudge_param_lower,
    # ## upper y limit
    y_range[2] + y_range_diff * nudge_param_upper
  )
  return(coords)
}
