...
data <- matrix(sample(0:1, num_rows * num_columns, replace = TRUE), nrow = num_rows)

# Create a DataFrame
df_CERAD <- as.data.frame(data)
df_resp <- df_CERAD
#################
...
Dep <- matrix(0, 30, 30) # Build 30x30 matrix of zeros
temp <- rep(1:10, 6)
for(i in 1:30) {
  Dep[i, which(item_ident[11:30] == temp[i])+10] <- 1
  print(which(item_ident[11:30] == temp[i])+10)
}
Dep[11:30, 11:20] <- 0
Dep[1:10, 21:30] <- 0
Dep[21:30, 11:30] <- 0
...
# Question: It looks to me as if the dependency matrix is one off? Did I missing something?
## - It makes sense for the first ten rows
## - Ask?
