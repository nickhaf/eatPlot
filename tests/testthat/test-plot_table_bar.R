test_that("Table-barplot (trend_books) is still the same", {
  ...
  p2 <- plot_bar(plot_data[["plot_bar"]], plot_settings = plotsettings_barplot(bar_sig_type = "pattern", default_list = barplot_MinSta))
...
test_that("Table-barplot (trend_books) is still the same with frames", {
  plot_data <- prep_no_trend(dat = adjusted_means,
                             ...
                             sig_niveau = 0.05)
  p1 <- plot_table(plot_data[["plot_table"]])
  p2 <- plot_bar(plot_data[["plot_bar"]], plot_settings = plotsettings_barplot(bar_sig_type = "frame", default_list = barplot_MinSta))
