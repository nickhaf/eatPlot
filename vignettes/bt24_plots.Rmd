...
trend_3_subset$group <- subset(trend_3_subset$group, trend_3_subset$group$mhg %in% c("einET", "ersteGen") | is.na(trend_3$group$mhg))
trend_3_subset$group$TR_BUNDESLAND <- factor(trend_3_subset$group$TR_BUNDESLAND)
...
trend_3_prepped <- prep_lineplot(
  ...
  background_group = NA, ## group of the background-variable, that is only plotted as background line
  # line_se = "trend",
